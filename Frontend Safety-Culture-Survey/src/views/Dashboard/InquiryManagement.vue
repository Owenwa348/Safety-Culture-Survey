    ...
    <div class="ml-64 p-6">
      ...
      <div v-else-if="inquiries.length > 0" class="bg-white rounded-lg shadow overflow-hidden">
          ...
          <table class="w-full">
            ...
            <tbody>
              <tr v-for="inquiry in inquiries" :key="inquiry.id" class="border-b hover:bg-gray-50">
                <td class="px-6 py-3 text-sm text-gray-700 whitespace-nowrap">{{ formatDate(inquiry.createdAt) }}</td>
                ...
                <td class="px-6 py-3 text-sm">
                  <div class="flex space-x-2">
                    <button @click="openModal(inquiry)" class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600 transition">ดู</button>
                    <button @click="deleteInquiry(inquiry.id)" class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 transition">ลบ</button>
                  </div>
                </td>
              </tr>
            </tbody>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center p-6 bg-gray-50 border-t gap-4">
          ...
          <div class="flex space-x-2">
            <button @click="previousPage" :disabled="currentPage === 1" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400 transition disabled:opacity-50 disabled:cursor-not-allowed">ก่อนหน้า</button>
            <button @click="nextPage" :disabled="currentPage === totalPages" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400 transition disabled:opacity-50 disabled:cursor-not-allowed">ถัดไป</button>
          </div>
        </div>
      </div>
    ...
    <div v-if="selectedInquiry && showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div class="bg-[#7ae2cf] text-black px-6 py-4 flex justify-between items-center sticky top-0 z-10">
          ...
          <button @click="closeModal" class="text-2xl font-bold hover:opacity-70 transition">✕</button>
        </div>
        ...
        <div class="p-6 space-y-4">
          ...
          <div>
            ...
            <div class="bg-gray-50 p-4 rounded text-gray-800 whitespace-pre-wrap break-words">{{ selectedInquiry.message }}</div>
          </div>
        </div>

        <div class="bg-gray-50 px-6 py-4 flex justify-end border-t sticky bottom-0">
          <button @click="closeModal" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400 transition">ปิด</button>
        </div>
      </div>
    </div>
  </div>
...
const fetchInquiries = async () => {
  ...
  try {
    ...
    if (yearToFilter) url.searchParams.append("year", yearToFilter);
    if (selectedMonth.value) url.searchParams.append("month", selectedMonth.value);
...
const deleteInquiry = async (id) => {
  if (!confirm("คุณแน่ใจหรือว่าต้องการลบติดต่อสอบถามนี้?")) return;
  try {
    const response = await fetch(`${API_BASE_URL}/api/inquiry/${id}`, { method: "DELETE" });
