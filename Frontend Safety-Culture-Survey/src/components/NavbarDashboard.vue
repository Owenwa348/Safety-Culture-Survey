<template>
  <div>
    ...
    <button
      ...
      class="fixed top-4 left-4 z-[70] md:hidden flex items-center justify-center w-9 h-9 bg-[#06202B] text-white rounded-lg shadow-lg"
      aria-label="เปิดเมนู"
    >
    ...
    <!-- Sidebar -->
    <nav
      class="fixed top-0 left-0 h-full w-64 bg-gradient-to-b from-[#06202B] to-[#041419] shadow-2xl flex flex-col z-[65] transition-transform duration-300"
    ...
    >
      ...
      <div class="flex-1 overflow-y-auto py-6 px-3 custom-scrollbar">
        <div class="space-y-2">
          ...
          <div :class="{ 'opacity-40 pointer-events-none': isSuperAdmin }">
            ...
            <div v-show="isAssessmentMenuOpen" class="mt-1 ml-8 space-y-1 border-l-2 border-[#7AE2CF]/30 pl-3">
              <router-link to="/workgroup-evaluation-results" @click="isSidebarOpen = false"
              ...
              <router-link to="/question-results" @click="isSidebarOpen = false"
                class="block text-sm px-4 py-2.5 rounded-lg transition-all duration-200"
              ...
              <router-link to="/opinion-results" @click="isSidebarOpen = false"
                class="block text-sm px-4 py-2.5 rounded-lg transition-all duration-200"
          ...
          <div v-if="isSuperAdmin">
            ...
            <div v-show="isSettingsMenuOpen" class="mt-1 ml-8 space-y-1 border-l-2 border-[#7AE2CF]/30 pl-3">
              <router-link to="/settings" @click="isSidebarOpen = false"
                class="block text-sm px-4 py-2.5 rounded-lg transition-all duration-200"
              ...
              <router-link to="/superadmin" @click="isSidebarOpen = false"
                class="block text-sm px-4 py-2.5 rounded-lg transition-all duration-200"
              ...
              <router-link to="/excelupload" @click="isSidebarOpen = false"
                class="block text-sm px-4 py-2.5 rounded-lg transition-all duration-200"
              ...
              <router-link to="/inquiry-management" @click="isSidebarOpen = false"
                class="block text-sm px-4 py-2.5 rounded-lg transition-all duration-200"
      ...
      <div class="border-t border-gray-700/30 bg-black/20">
        ...
        >
          <div class="w-11 h-11 bg-gradient-to-br from-[#7AE2CF] to-[#4ECDC4] rounded-full flex items-center justify-center font-bold text-[#06202B] shadow-lg ring-2 ring-[#7AE2CF]/20 group-hover:ring-[#7AE2CF]/40 transition-all flex-shrink-0">
            {{ userInitials }}
          </div>
          <div class="flex-1 ml-3 text-left min-w-0">
            ...
            <div class="text-xs text-gray-400 truncate flex items-center gap-1.5">
              <span class="inline-block w-1.5 h-1.5 rounded-full bg-[#7AE2CF] flex-shrink-0"></span>
              {{ displayPosition }}
            </div>
          </div>
          <svg
          ...
          >
            <div class="px-4 py-3.5 bg-gradient-to-r from-[#06202B] to-[#0a3d52]">
              <div class="flex items-center gap-3">
                <div class="w-12 h-12 bg-gradient-to-br from-[#7AE2CF] to-[#4ECDC4] rounded-full flex items-center justify-center text-[#06202B] font-bold shadow-md ring-2 ring-white/20 flex-shrink-0">
                  {{ userInitials }}
                </div>
...
const isAssessmentActive = computed(() =>
  ['/workgroup-evaluation-results', '/question-results', '/opinion-results'].includes(route.path)
)

const isSettingsActive = computed(() =>
  ['/settings', '/admin', '/superadmin', '/excelupload', '/inquiry-management'].includes(route.path)
)
...
const displayPosition = computed(() => {
  switch (userData.value.role) {
    case 'Admin': return userData.value.company || 'Admin'
    case 'SuperAdmin': return 'ผู้ดูแลระบบ'
    default: return userData.value.position || 'ผู้ใช้งาน'
  }
})

const roleStyleClass = computed(() => {
  switch (userData.value.role) {
    case 'SuperAdmin': return 'bg-red-50 text-red-700 border border-red-200'
...
onMounted(() => {
  loadUserData()
  if (isAssessmentActive.value) isAssessmentMenuOpen.value = true
  if (isSettingsActive.value) isSettingsMenuOpen.value = true
})
</script>
...
.custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(122,226,207,0.3); border-radius: 10px; }
.custom-scrollbar::-webkit-scrollbar-thumb:hover { background: rgba(122,226,207,0.5); }
</style>